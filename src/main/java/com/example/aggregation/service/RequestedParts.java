...
public record RequestedParts(boolean all, Set<String> names) {

    private static final String INCLUDE_FIELD = "include";

    public static RequestedParts from(JsonNode inboundRequest) {
        JsonNode includeNode = inboundRequest.path(INCLUDE_FIELD);
        if (includeNode.isMissingNode() || includeNode.isNull()) {
            return new RequestedParts(true, Set.of());
        }

        if (!includeNode.isArray()) {
            throw new IllegalArgumentException("'include' must be an array of aggregation part names");
        }
        ...
        return new RequestedParts(false, Collections.unmodifiableSet(requested));
    }
